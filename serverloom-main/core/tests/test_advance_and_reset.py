...
class AdvanceAndResetTests(TestCase):
    ...
    def test_reset_weekly_salary_is_idempotent_and_creates_history(self):
        ...
        result_first = services.archive_and_reset_weekly_salaries(for_date=today, admin_user=None, notes="test reset")
        self.assertEqual(result_first["created"], 1)
        self.assertTrue(SalaryHistory.objects.filter(employee=self.emp, week_start=monday, week_end=sunday).exists())
        # calling again should not create duplicate rows; quantities simply refresh to identical values
        result_second = services.archive_and_reset_weekly_salaries(for_date=today, admin_user=None, notes="test reset")
        self.assertEqual(result_second["created"], 0)  # idempotent
