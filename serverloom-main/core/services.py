...
@transaction.atomic
def archive_and_reset_weekly_salaries(for_date: Optional[date] = None, admin_user: Optional[User] = None, notes: str = "") -> dict:
    """
    Freeze this week's salaries into SalaryHistory and zero 'current_week_salary' per employee.

    Weekly-record rule (Phase 4.5 stabilization, resolves BUG-17):
    - The archive command is the single authority for weekly QUANTITIES
      (sarees, salary_rate, total_salary_before_advance, advance_salary, final_salary).
      For every employee it creates the week's row if missing, or REFRESHES those
      quantity fields to exact end-of-week canonical values if a row already exists
      (e.g. one created by an earlier mark-paid click).
    - Mark-Paid / Mark-Unpaid are the authorities for PAYMENT STATE only
      (paid_status, paid_date, notes). Archive never changes payment state:
      a genuinely paid week stays paid; an unpaid week stays unpaid.
    - Idempotent: re-running for the same week recomputes the same values and
      reports 0 rows created (refreshes are no-ops numerically).

    Returns {"created": n_created, "updated": n_updated}.
    ...
    for emp in employees:
        ...
        if sh is None:
            ...
            # Refresh quantities to end-of-week truth; payment state is untouched.
            sh.sarees = numbers["sarees"]
            sh.salary_rate = numbers["salary_rate"]
            sh.total_salary_before_advance = numbers["total_before_advance"]
            sh.advance_salary = numbers["advance_applied"]
            sh.final_salary = numbers["final_salary"]
            sh.save(update_fields=[
                "sarees", "salary_rate", "total_salary_before_advance",
                "advance_salary", "final_salary", "updated_at",
            ])
            updated += 1
...
@transaction.atomic
def finish_warp(warp_id: int, admin_user: Optional[User] = None, note: str = "") -> WarpHistory:
    """
    Finish a WarpHistory (set end_date to today) and record a WarpChangeHistory audit row.
    Mirrors finish_pagdi so both material lifecycles share the same discipline (BUG-10).
    Uses select_for_update on the WarpHistory record.
    """
    from .models import WarpChangeHistory

    w = WarpHistory.objects.select_for_update().get(id=warp_id)
    prev_end = w.end_date
    w.end_date = timezone.localdate()
    w.save(update_fields=["end_date"])
    WarpChangeHistory.objects.create(
        ...
        admin_user=admin_user,
        action="FINISH",
        previous_capacity=w.capacity_sarees,
        new_capacity=w.capacity_sarees,
        previous_end_date=prev_end,
        new_end_date=w.end_date,
