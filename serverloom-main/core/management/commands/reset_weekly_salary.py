        ...
        if dry:
            ...
            try:
                with transaction.atomic():
                    result = services.archive_and_reset_weekly_salaries(for_date=use_date, admin_user=None, notes=note)
                ...
                if str(e) == "DRY_RUN_ROLLBACK":
                    self.stdout.write(self.style.SUCCESS(
                        f"DRY RUN: would create {result['created']} SalaryHistory rows and refresh {result['updated']} existing rows (no DB changes)."
                    ))
                    return
                raise
        else:
            result = services.archive_and_reset_weekly_salaries(for_date=use_date, admin_user=None, notes=note)
            self.stdout.write(self.style.SUCCESS(
                f"Archived and reset weekly salaries. SalaryHistory rows created: {result['created']}, refreshed: {result['updated']}"
            ))
